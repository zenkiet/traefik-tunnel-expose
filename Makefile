...
help:
	...
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)build$(RESET)             $(DIM)Build Docker image with optimization$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)multi-arch-build$(RESET)  $(DIM)Build multi-architecture image$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)push$(RESET)              $(DIM)Push image to Docker Hub$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)release$(RESET)           $(DIM)Create and push versioned release$(RESET)"
	@echo "$(YELLOW)$(BOLD)└───$(RESET)"
	@echo ""
	@echo "$(YELLOW)$(BOLD)┌─ 🚀 Service Management$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)dev$(RESET)               $(DIM)Start development environment$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)prod$(RESET)              $(DIM)Start production environment$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)run$(RESET)               $(DIM)Start services with auto-restart$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)stop$(RESET)              $(DIM)Stop all services gracefully$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)restart$(RESET)           $(DIM)Zero-downtime service restart$(RESET)"
	@echo "$(WHITE)│$(RESET)  $(GREEN)$(BOLD)update$(RESET)            $(DIM)Update to latest images$(RESET)"
	@echo "$(YELLOW)$(BOLD)└───$(RESET)"
	@echo ""
	...
	@echo "  $(WHITE)make setup && make dev$(RESET)     $(DIM)# Setup and start development$(RESET)"
	@echo "  $(WHITE)make build VERSION=v1.0.0$(RESET) $(DIM)# Build with specific version$(RESET)"
	@echo "  $(WHITE)make ci && make cd$(RESET)         $(DIM)# Full CI/CD pipeline$(RESET)"
	@echo ""
...
## Build Docker image with caching and optimization
build:
	...
	@docker build \
		...
		--cache-from $(DOCKER_NAMESPACE)/$(IMAGE_NAME):latest \
		$(BUILD_ARGS) \
		$(DOCKER_CONTEXT)
...
check-buildx:
	@docker buildx version >/dev/null 2>&1 || ($(call print_error,Docker buildx not available) && exit 1)

## Push image to Docker Hub with security scanning
push: build security-scan
...
## Create and push release with tags
release: build test security-scan push
...
dev: setup
	...
	@COMPOSE_PROFILES=dev docker-compose -f $(COMPOSE_FILE) up -d
	$(call print_success,Development environment started)
	@echo "$(CYAN)📊 Traefik Dashboard: http://localhost:8080$(RESET)"
	@echo "$(CYAN)📝 Development tools available$(RESET)"

## Start services in production mode
prod: setup validate-config
	$(call print_step,Starting production environment)
	@COMPOSE_PROFILES=prod docker-compose -f $(COMPOSE_FILE) up -d
	$(call print_success,Production environment started)

## Start services with auto-restart
run: setup
	$(call print_step,Starting services with auto-restart)
...
stop:
	$(call print_step,Stopping services gracefully)
	@docker-compose down --timeout 30
	$(call print_success,Services stopped successfully)

## Restart services with zero-downtime
restart:
	$(call print_step,Performing zero-downtime restart)
	@docker-compose up -d --force-recreate --remove-orphans
	$(call print_success,Services restarted successfully)

## Update all services to latest images
update:
	$(call print_step,Updating services to latest images)
	@docker-compose pull
	@docker-compose up -d --remove-orphans
	@docker image prune -f
	$(call print_success,Update completed successfully)
...
logs-traefik:
	@docker-compose logs -f --timestamps traefik-tunnel

## View error logs only
logs-errors:
	@docker-compose logs --timestamps | grep -i error
