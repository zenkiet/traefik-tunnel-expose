...
import { readConfig, writeConfig, deleteConfig } from "$lib/server/files";
...
export const GET: RequestHandler = async ({ params }) => {
  ...
  const content = await readConfig(entry.filePath);
...
export const PUT: RequestHandler = async ({ params, request }) => {
  ...
  await writeConfig(entry.filePath, payload.content);
...
export const DELETE: RequestHandler = async ({ params }) => {
  ...
  if (
    ...
    entry.id !== "traefik" &&
    entry.id !== "cloudflared"
  ) {
    throw error(400, "Deletion not allowed for this file");
  }

  await deleteConfig(entry.filePath);

  return json({ ok: true });
};

function toPublicEntry(
  entry: NonNullable<Awaited<ReturnType<typeof resolveEntry>>>,
) {
